...
from dataset_preparation import (
    _fill_scores,
    _find_closest_coordinates,
    _find_overlapping_coordinates,
    _overlap,
    add_median_column,
    clean_coordinates,
    clean_scores,
    combine_CDA_dataframes,
    get_central_scorer,
    read_CDAScorer_data,
    save_cropped_images,
)
...
class TestReadCDAScorerData:
    ...
    def test_valid_folder_with_csv_files(self, tmp_path: Path) -> None:
        ...
        """
        csv_content = (
            "img,maxrow,maxcol,row,col,pos,score,x1,x2,y1,y2\nimage1,5,5,1,1,1,10,10,20,20,30"
        )
...
class TestFillScores:
    ...
    def setup_data(self) -> tuple[dict[str, pd.DataFrame], pd.Series]:
        ...
        cda_dataframes = {"scorer1": scorer1_data, "scorer2": scorer2_data}
    ...
    def test_fill_scores_valid_data(
        self, setup_data: tuple[dict[str, pd.DataFrame], pd.Series]
    ) -> None:
        """
    ...
    def test_fill_scores_valid_data_scorer2(
        self, setup_data: tuple[dict[str, pd.DataFrame], pd.Series]
    ) -> None:
        """
        Test _fill_scores function with valid data for scorer number 2
        (different columns to scorer number 1)
    ...
    def test_fill_scores_invalid_scorer_number(
        self, setup_data: tuple[dict[str, pd.DataFrame], pd.Series]
    ) -> None:
        """
...
class TestCombineCDADataframes:
    ...
    def test_invalid_scorer_info_path(
        self, setup_data: tuple[dict[str, pd.DataFrame], Path]
    ) -> None:
        """
...
class TestAddMedianColumn:
    ...
    def test_add_median_column_correct_data(self, setup_data: pd.DataFrame) -> None:
        """
        Test add_median_column function with correct data to ensure the median
        column is calculated correctly.
    ...
    def test_add_median_column_with_na(self, setup_data: pd.DataFrame) -> None:
        """
        Test add_median_column function with a missing value to ensure the median
        is correctly calculated with NA.
...
class TestFindClosestCoordinates:
    ...
    def coord_lists(self) -> list[list[float]]:
        """
        Fixture to provide a list of coordinate sets for testing the
        find_closest_coordinates function.
    ...
    def test_find_closest_coordinates_equally_close(self, coord_lists: list[list[float]]) -> None:
        """
        Test find_closest_coordinates function where two coordinate sets are
        equally as close to the centre and are different sizes
...
class TestGetCentralScorer:
    ...
    def test_get_central_scorer_with_na(self, coord_fixture: pd.Series) -> None:
        ...
        -----------
        coord_fixture : pd.Series
                A pandas Series containing coordinates for rectangles, with one
                set containing NA values.
...
class TestSaveCroppedImages:
    ...
    def test_save_cropped_images(
        ...
        This test simulates reading and writing images using mocks and asserts
        that the correct functions (imread, imwrite, and print) are called as expected.
