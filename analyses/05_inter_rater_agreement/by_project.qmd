...
### 1.2 Load data and add project columns
...
### 1.3 Prepare long score tables and pairwise agreement
...
## 2. CDA count per image by project
...
## 3. Per-image agreement, size, and zeros by project
...
### 3.1 Agreement with median per image
...
### 3.3 Agreement vs proportion of 0s
...
### 3.4 Average agreement with median per project (with 95% CI)

```{r fig.width=10, fig.height=6}
#| fig-width: 10
#| fig-height: 6
# 3.4
# Bars show the mean agreement per project (images as the unit of
# replication); error bars are the 95% CI (t-distribution, using each
# project's own image count); points are the individual per-image values,
# jittered.
...
## 4. Images per rater faceted by project
...
## 5. Average pairwise agreement per project
...
## 6. Expected vs actual CDA counts

### 6.1 Estimated vs actual CDAs per source image
...
### 6.2 Expected count per rater pair (assumes no dual-infiltration)
...
## 7. Sensitivity analysis: removing projects

```{r}
# 7.1
# Each project is removed in turn (along with any two-rater CDAs that then
# disagree as a result), and the overall pairwise agreement is recomputed.
...
## 8. Score class distribution by project (heatmap)

```{r fig.width=10, fig.height=7}
#| fig-width: 10
#| fig-height: 7
# 8
# Proportion of CDA severity classes (median score) within each project,
# mirroring the rater heatmap in 02_score_distributions.qmd 3.2.
...
- Most images had between 21-30 cell death areas.
...
- The average agreement per project ties directly into the agreement vs proportion-of-0s plot in §3.3.
- The frequencies are below what we would expect.

_Add notes here._

## 10. Additional Visualisation

### 10.1 Dual-infiltration image example
