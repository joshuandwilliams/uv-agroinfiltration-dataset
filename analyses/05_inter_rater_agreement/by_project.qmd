---
title: "Project differences between raw images"
format:
  html:
    code-fold: true
---
...
Are there differences in CDA counts per raw agro image between different projects, and do those projects differ in how consistently they were scored?
...
There will be differences in CDA counts per raw agro image, and in scoring agreement, between different projects.

**Rationale:**

Part 1 of the CDA dataset contains some dual-infiltration CDAs, which were ignored during scoring but not during raw agro image distribution. Part 2 does not.

Different projects used different biological systems, different numbers of leaves per raw image, and different numbers of infiltrations per leaf.
...
save_plot <- function(p, filename, width, height, dir = "supp_plots") {
  ggsave(here("analyses", "05_inter_rater_agreement", dir, filename),
...
alloc_path <- here("analyses", "01_generate_raw_data", "01_allocation_info", "randomised_info.csv")
...
Convert to long form (one row per scorer assignment), with anonymised scorer IDs.
...
anon_key_path <- here("analyses", "01_generate_raw_data", "02_combining_cdascorer_outputs", "anonymisation_key.csv")
anon_key <- read_csv(anon_key_path, show_col_types = FALSE)
name_to_id <- setNames(as.character(anon_key$ID), anon_key$Scorer_Name)

allocation_long_1 <- allocation_long(allocation_data_1) %>% mutate(ScorerName = name_to_id[ScorerName])
allocation_long_2 <- allocation_long(allocation_data_2) %>% mutate(ScorerName = name_to_id[ScorerName])
...
pairwise_score_data <- function(score_data_long) {
  ...
  all_combinations <- expand.grid(
    Scorer.x = unique(agreement_data$Scorer.x), Scorer.y = unique(agreement_data$Scorer.y)
  )

  merged_df <- all_combinations %>%
    left_join(agreement_data, by = c("Scorer.x", "Scorer.y")) %>%
    mutate(
      Total = ifelse(is.na(Total), NA, Total),
...
p <- ggplot(image_stats, aes(x = Avg_Percent_Agreement, fill = Folder)) +
  ...
  labs(x = "Average % Agreement with Median", y = "Number of Images") +
  theme_minimal() +
  theme(
    axis.text = element_text(size = 13),
    axis.title = element_text(size = 15),
...
p <- ggplot(image_stats, aes(x = Percent_0, y = Avg_Percent_Agreement, color = Folder)) +
  ...
  labs(x = "% of scores that were 0", y = "Average % Agreement with Median") +
  theme_minimal() +
  theme(
    axis.text = element_text(size = 13),
    axis.title = element_text(size = 15),
...
p <- ggplot(project_agreement, aes(x = Folder, y = Mean)) +
  ...
  labs(x = "Project", y = "Average % Agreement with Median") +
  ylim(0, 100) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
...
## 5. Images per scorer faceted by project
...
p <- ggplot(count_data, aes(x = ScorerName, y = row_count, color = ScorerName)) +
  ...
  labs(x = "Scorer", y = "Agroinfiltration Image Count", color = "Scorer ID") +
  theme_minimal() +
  theme(
...
### 7.1 Estimated vs actual CDAs per raw image
...
### 7.2 Expected count per scorer pair (assumes no dual-infiltration)
...
p <- ggplot(pairwise_combined, aes(x = Set, y = Total)) +
  ...
  scale_y_continuous(limits = c(0, NA)) +
  labs(x = "Dataset", y = "CDA Count per Scorer Pair") +
  theme_minimal() +
  theme(
    axis.text = element_text(size = 13),
    axis.title = element_text(size = 15),
...
Each project is removed in turn (along with any two-scorer CDAs that then disagree), and the overall pairwise agreement is recomputed.
...
Proportion of CDA severity classes (median score) within each project, mirroring the scorer heatmap in `02_score_distributions.qmd` §3.2.
