...
# Set1 only has 9 qualitative colours; with 10 raters we add one more
# (black) to keep every rater visually distinct.
rater_levels <- levels(count_data$ScorerName)
rater_pal <- setNames(c(RColorBrewer::brewer.pal(9, "Set1"), "black")[seq_along(rater_levels)], rater_levels)
...
p <- ggplot(count_data, aes(x = ScorerName, y = row_count, color = ScorerName)) +
  ...
  scale_colour_manual(values = rater_pal) +
...
p_top <- ggplot() +
  ...
  labs(x = NULL, y = "Corrected agreement") +
  theme_minimal() +
  theme(
...
p_bottom <- ggplot(image_stats_nonzero, aes(x = Folder, y = Nonzero_Agreement, fill = Folder)) +
  ...
  scale_fill_brewer(palette = "Set1", guide = "none") +
  labs(x = "Project", y = "Corrected agreement (outside score class 0)") +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    axis.text = element_text(size = 13),
...
The value at the centre (median) of each boxplot in panels A and B, alongside
the panel A zero-proportion overlay, in the same left-to-right project order as
the figure:

```{r}
# 6.2 (table) - boxplot medians behind panels A and B, plus the overlay
panel_medians <- image_stats_ordered %>%
  group_by(Folder) %>%
  summarise(A_median = median(Avg_Percent_Agreement), .groups = "drop") %>%
  left_join(zero_by_project, by = "Folder") %>%
  left_join(
    image_stats_nonzero %>%
      group_by(Folder) %>%
      summarise(B_median = median(Nonzero_Agreement), .groups = "drop"),
    by = "Folder"
  ) %>%
  arrange(Folder) %>%
  transmute(
    Project = as.character(Folder),
    `A: median agreement (%)` = round(A_median, 1),
    `A: % scores = 0` = round(Percent_0, 1),
    `B: median agreement, non-zero scores (%)` = round(B_median, 1)
  )

panel_medians %>%
  knitr::kable(
    caption = "Boxplot medians for panels A and B, and the panel A zero-proportion overlay, per project"
  )
```
