...
def read_CDAScorer_data(folder_path: str | Path) -> dict[str, pd.DataFrame]:
    """
...
def _fill_scores(
    cda_dataframes: dict[str, pd.DataFrame], row: pd.Series, scorer_number: int
) -> list[int | None]:
    """
    ...
    dataframes : Dict[str, pd.DataFrame]
            A dictionary where keys are scorer names and values are DataFrames
            containing the scoring data.

    row : pd.Series
            A row from the output DataFrame that contains the information about
            Basename, Row, Col, Pos, and Scorer.
...
def combine_CDA_dataframes(
    cda_dataframes: dict[str, pd.DataFrame], scorer_info: str | Path
) -> pd.DataFrame:
    """
    ...
    for key in cda_dataframes:
        cda_dataframes[key]["img"] = cda_dataframes[key]["img"].apply(
    ...
    unique_combinations = (
        combined_df[["img", "row", "col", "pos"]]
        .drop_duplicates()
        .sort_values(by=["img", "row", "col", "pos"])
    )
    ...
    output_table = output_table.merge(
        randomised_info[["basename", "scorer1", "scorer2", "scorer3"]],
        left_on="img",
        right_on="basename",
        how="left",
    )
    ...
    for i in range(1, 4):
        cols = [f"score{i}", f"x1_{i}", f"x2_{i}", f"y1_{i}", f"y2_{i}"]
        output_table[cols] = output_table.apply(
            lambda row, i=i: pd.Series(_fill_scores(cda_dataframes, row, i)), axis=1
        )
...
def clean_coordinates(row: pd.Series) -> pd.Series:
    ...
    for index, coord_set in enumerate(coords):
        ...
        coord_set[0], coord_set[1] = min(coord_set[0], coord_set[1]), max(
            coord_set[0], coord_set[1]
        )
        coord_set[2], coord_set[3] = min(coord_set[2], coord_set[3]), max(
...
def clean_scores(data: pd.DataFrame) -> pd.DataFrame:
    ...
    disagreement_mask = (data[["Score1", "Score2", "Score3"]].isnull().sum(axis=1) == 1) & (
...
def add_median_column(data: pd.DataFrame) -> pd.DataFrame:
    ...
    -----------
    data : pd.DataFrame
            The dataframe containing the 3 score columns (Score1, Score2, Score3)
...
def _overlap(rect1: list[int], rect2: list[int]) -> bool:
    ...
    Each rectangle is represented by a list of four coordinates [x1, x2, y1, y2].
...
def _find_overlapping_coordinates(coord_lists: list[list[int]]) -> list[list[int]] | None:
    """
    ...
    coord_lists: List[int]
            A variable number of lists, each representing 4 coordinates of a
    ...
    overlapping_sets = [
        list(coords) for coords in set(tuple(coords) for coords in overlapping_sets)
    ]
...
def _find_closest_coordinates(coord_lists: list[list[float]]) -> list[float] | None:
    """
    ...
    --------
    Optional[List[float]]
            The rectangle coordinates closest to the center of the overlapping area,
            or None if no overlapping area is found.
    """
    ...
    overlapping_center = [
        (overlapping_area[0] + overlapping_area[1]) / 2,
        (overlapping_area[2] + overlapping_area[3]) / 2,
    ]
...
def get_central_scorer(row: pd.Series) -> int | None:
    """
    Find the index of the coordinate set closest to the centre of the overlapping
    area between all valid coordinate sets for that CDA.
    ...
    --------
    Optional[int]
            The index of the rectangle (1, 2, or 3) that is closest to the center
            of the overlapping area. Returns None if no overlapping rectangles are found.
    """
    coord1 = [row["X1_1"], row["X2_1"], row["Y1_1"], row["Y2_1"]]
    coord2 = [row["X1_2"], row["X2_2"], row["Y1_2"], row["Y2_2"]]
    coord3 = [row["X1_3"], row["X2_3"], row["Y1_3"], row["Y2_3"]]
    ...
    for coord, index in zip(coord_lists, indices, strict=False):
...
def save_cropped_images(data: pd.DataFrame, out_folder: Path) -> None:
    """
    Saves cropped images from the input DataFrame based on given coordinates and
    scorer information.

    Parameters:
    -----------
    data : pd.DataFrame
            DataFrame containing image information such as 'Centre_Coords',
            'Basename', scorer columns, and crop coordinates.
    out_folder: Path
    ...
    --------
    None
            This function does not return anything but saves cropped images to
    ...
    for _, row in data.iterrows():
        ...
        coords = [
            row[f"X1_{centre_coords}"],
            row[f"X2_{centre_coords}"],
            row[f"Y1_{centre_coords}"],
            row[f"Y2_{centre_coords}"],
        ]
        ...
        if not current_file == previous_file:
            paths = [
                os.path.join(
                    "/Volumes/shared/Research-Groups/Mark-Banfield/Josh_Williams/Scoring_20Oct",
                    row[f"Scorer{centre_coords}"],
                    row["Basename"],
                ),
                os.path.join(
                    "/Volumes/shared/Research-Groups/Mark-Banfield/Josh_Williams/Scoring_5May",
                    row[f"Scorer{centre_coords}"],
                    row["Basename"],
                ),
            ]
