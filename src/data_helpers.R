...
#' to anonymisation_keys/anonymisation_key.csv.
#'
...
load_scorer_key <- function(as_character = FALSE, path = NULL) {
  if (is.null(path)) {
    path <- here::here("anonymisation_keys", "anonymisation_key.csv")
  }
...
#'
#' @param path Path to the key CSV. Defaults to the canonical repo path
#'   (anonymisation_keys/project_anonymisation_key.csv); overridable for
#'   testing.
#' @return A data frame with columns Project, Anon_Label, Dual_Infiltration.
load_project_key <- function(path = NULL) {
  if (is.null(path)) {
    path <- here::here("anonymisation_keys", "project_anonymisation_key.csv")
  }
