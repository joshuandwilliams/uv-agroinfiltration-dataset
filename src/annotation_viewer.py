...
def _draw_score_label_pil(draw, row, label_x, label_y, font, font_size):
    ...
    for j, (score, color) in enumerate(zip(scores, [SCORER_COLORS_PIL[i] for i in range(1, 4)])):
...
def _draw_score_label_tk(canvas, row, label_x, label_y, tk_font_obj):
    ...
    for j, (score, color) in enumerate(zip(scores, [SCORER_COLORS_TK[i] for i in range(1, 4)])):
        sequence.append((str(int(score)) if not pd.isna(score) else '-', color))
        if j < 2:
    ...
    cursor = label_x
    for text, color in sequence:
        canvas.create_text(cursor, label_y, text=text, fill=color,
